...
local function testlocation(inst, pt)
	...
	--	return false
	--end
	if(  (testTile == GROUND.MANGROVE) or (testTile == GROUND.MAGMAFIELD) or (testTile >= GROUND.OCEAN_SHALLOW and testTile <= GROUND.LILYPOND) or (testTile > GROUND.UNDERGROUND) or ( testTile == GROUND.IMPASSABLE) ) then
		return false
	end	
