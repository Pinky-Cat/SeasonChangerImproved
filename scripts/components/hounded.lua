...
local function getHoundPrefab(inst)
	...
	if sm:IsTropical() then
		if math.random() < special_hound_chance then	
			if GetWorld().components.flooding and (GetWorld().components.flooding.mode == "flood" or sm:IsWinter() or sm:IsHumidSeason()) then
				prefab = "watercrocodog"				
				ambibious = true
			elseif sm:IsDrySeason() or sm:IsSummer() or sm:IsLushSeason() then
		...
		if math.random() < special_hound_chance then
		    if (sm:IsWinter() or sm:IsSpring() or sm:IsHumidSeason() or sm:IsGreenSeason()) then
		        prefab = "icehound"
			elseif (sm:IsDrySeason() or sm:IsSummer() or sm:IsLushSeason()) then
			    prefab = "firehound"
			end
