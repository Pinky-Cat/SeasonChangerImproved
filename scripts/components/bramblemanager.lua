...
local BrambleManager = Class(function(self, inst)
	...
	if seasonmgr:IsPlateau() then			
		    self.bramblesToSpawn = 7
	elseif seasonmgr:IsTropical() then		--More brambles in SW	
		    self.bramblesToSpawn = 50
	else		
			self.bramblesToSpawn = 23		--More brambles on ROG
	end
...
function BrambleManager:SpawnBrambles()
	...
	for i=#tempselctionset,1,-1 do
		...
		if  (tile == GROUND.IMPASSABLE or tile >= GROUND.MANGROVE) then
			print("REMOVING SPAWN OPTION",tile)
			table.remove(tempselctionset,i)
		end
