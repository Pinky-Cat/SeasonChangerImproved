...
local SeasonManager = Class(function(self, inst)
	...
	self.current_temperature_last =10
	self.current_temperature_init =0
	self.noise_time = 0	
	...
	--------------------Season Changer Variables---------------------
	self.season_current_number = 0
	self.season_limit_number = 27 -- This number will be one below the max amount of seasons in the mod's config. Change this number when changing the amount of seasons in config as it governs when SeasonManager should loop back to the first season.
	self.seasonlist= {}
	self.season_mod_enabled = 0
...
-- Where the mod update seasons. --

function SeasonManager:InitSeasonsO()
	local foldername = KnownModIndex:GetModActualName("Seasons Changer Improved")
	self.season_current_number = self.season_current_number or 0
	
	--[[ Old Version, hard coded values for each index of seasonList.
	self.t_season_1 = self.t_season_1 or GetModConfigData("SEASON_1", foldername) -- Need to check why the t variable is first
	self.t_season_2 = self.t_season_2 or GetModConfigData("SEASON_2", foldername)
	self.t_season_3 = self.t_season_3 or GetModConfigData("SEASON_3", foldername)
	self.t_season_4 = self.t_season_4 or GetModConfigData("SEASON_4", foldername)
	self.t_season_5 = self.t_season_5 or GetModConfigData("SEASON_5", foldername)
	self.t_season_6 = self.t_season_6 or GetModConfigData("SEASON_6", foldername)
	self.t_season_7 = self.t_season_7 or GetModConfigData("SEASON_7", foldername)

	self.t_s1_length = self.t_s1_length or GetModConfigData("SEASON_1_LENGTH", foldername)
	self.t_s2_length = self.t_s2_length or GetModConfigData("SEASON_2_LENGTH", foldername)
	self.t_s3_length = self.t_s3_length or GetModConfigData("SEASON_3_LENGTH", foldername)
	self.t_s4_length = self.t_s4_length or GetModConfigData("SEASON_4_LENGTH", foldername)
	self.t_s5_length = self.t_s5_length or GetModConfigData("SEASON_5_LENGTH", foldername)
	self.t_s6_length = self.t_s6_length or GetModConfigData("SEASON_6_LENGTH", foldername)
	self.t_s7_length = self.t_s7_length or GetModConfigData("SEASON_7_LENGTH", foldername)

	-- This means any other mentions of "Length" in SeasonManager.lua are ignored. Will not comment out the other mentions to keep the code running smoothly. --
	self:SetWinterLength(0)
	self:SetAutumnLength(0)
	self:SetSpringLength(0)
	self:SetSummerLength(0)
	self:SetMildLength(0)
	self:SetWetLength(0)
	self:SetGreenLength(0)
	self:SetDryLength(0)
	self:SetTemperateLength(0)
	self:SetLushLength(0)
	self:SetHumidLength(0)
	self:SetAporkalypseLength(0)
	
	self:UpdateLength(self.t_season_1,self.t_s1_length)
	self:UpdateLength(self.t_season_2,self.t_s2_length)
	self:UpdateLength(self.t_season_3,self.t_s3_length)
	self:UpdateLength(self.t_season_4,self.t_s4_length)
	self:UpdateLength(self.t_season_5,self.t_s5_length)
	self:UpdateLength(self.t_season_6,self.t_s6_length)
	self:UpdateLength(self.t_season_7,self.t_s7_length)

			
	self.seasonlist[0]=self.t_season_1
	self.seasonlist[1]=self.t_season_2
	self.seasonlist[2]=self.t_season_3
	self.seasonlist[3]=self.t_season_4
	self.seasonlist[4]=self.t_season_5
	self.seasonlist[5]=self.t_season_6
	self.seasonlist[6]=self.t_season_7
	--]]
	if self.seasonList == nil then
		local season_random_options = {}
		if GetModConfigData("RANDOM_ALLOW_AUTUMN", foldername) then
				if GetModConfigData("RANDOM_ALLOW_AUTUMN", foldername) then
					table.insert(season_random_options, "autumn")
				end
				if GetModConfigData("RANDOM_ALLOW_WINTER", foldername) then
					table.insert(season_random_options, "winter")
				end
				if GetModConfigData("RANDOM_ALLOW_SPRING", foldername) then
					table.insert(season_random_options, "spring")
				end
				if GetModConfigData("RANDOM_ALLOW_SUMMER", foldername) then
					table.insert(season_random_options, "summer")
				end
				if GetModConfigData("RANDOM_ALLOW_MILD", foldername) then
					table.insert(season_random_options, "mild")
				end
				if GetModConfigData("RANDOM_ALLOW_WET", foldername) then
					table.insert(season_random_options, "wet")
				end
				if GetModConfigData("RANDOM_ALLOW_GREEN", foldername) then
					table.insert(season_random_options, "green")
				end
				if GetModConfigData("RANDOM_ALLOW_DRY", foldername) then
					table.insert(season_random_options, "dry")
				end
				if GetModConfigData("RANDOM_ALLOW_TEMPERATE", foldername) then
					table.insert(season_random_options, "temperate")
				end
				if GetModConfigData("RANDOM_ALLOW_HUMID", foldername) then
					table.insert(season_random_options, "humid")
				end
				if GetModConfigData("RANDOM_ALLOW_LUSH", foldername) then
					table.insert(season_random_options, "lush")
				end
				--[[if GetModConfigData("RANDOM_ALLOW_APORKALYPSE", foldername) then
					table.insert(season_random_options, "aporkalypse")
				end
				]]
				if GetModConfigData("RANDOM_ALLOW_NONE", foldername) then
					table.insert(season_random_options, "none")
				end
		end

		--The second number is the max limit of seasons. Change all references to it by searching !seasonLimit
		for i=1, self.season_limit_number+1, 1 do
			if GetModConfigData(string.format("SEASON_%d", i), foldername) then
				self.seasonlist[i] = {season_title = GetModConfigData(string.format("SEASON_%d", i), foldername), season_length = GetModConfigData(string.format("SEASON_%d_LENGTH", i), foldername)}
			end
			if self.seasonlist[i].season_title == "random" then
				self.seasonlist[i].season_title = season_random_options[math.random(#season_random_options)]
			end	
			if self.seasonlist[i].season_length == "random" then
				self.seasonlist[i].season_length = math.random(GetModConfigData("RANDOM_LENGTH_MIN", foldername), GetModConfigData("RANDOM_LENGTH_MAX", foldername))
			end
			-- Should random offset have min and max?
			if GetModConfigData("RANDOM_LENGTH_OFFSET", foldername) ~= 0 then
				self.seasonlist[i].season_length = self.seasonlist[i].season_length+math.random(-1*GetModConfigData("RANDOM_LENGTH_OFFSET", foldername), GetModConfigData("RANDOM_LENGTH_OFFSET", foldername))
				if self.seasonlist[i].season_length <= 0 then
					self.seasonlist[i].season_length = 1
				end
			end	 
			self:UpdateLength(self.seasonlist[i].season_title,self.seasonlist[i].season_length)
		end

	self:SetOverworld()
	--self:StartTemperate()
	--self:StartSeasonO(self.t_season_1)
	self:StartSeasonO(self.seasonlist[self.season_current_number+1].season_title)
	self:UpdateSegs()
	end
end
...
function SeasonManager:ChangeSeasonO()


	self.season_current_number = self.season_current_number + 1
	if self.season_current_number > self.season_limit_number then
			self.season_current_number = 0
		
	end
	while self.seasonlist[self.season_current_number+1].season_title == "none" do
	...
	--if(self.currSeason == SEASONS.APORKALYPSE) then
...
function SeasonManager:UpdateLength(season , season_length)
	
	if( season == nil or season_length == nil) then
		return
	end

	if(season == SEASONS.AUTUMN) then
			self:SetAutumnLength (season_length)
	elseif(season == SEASONS.WINTER) then
			self:SetWinterLength (season_length)

	elseif(season == SEASONS.SPRING) then
			self:SetSpringLength (season_length)

	elseif(season == SEASONS.SUMMER) then
			self:SetSummerLength (season_length)
	
	elseif(season == SEASONS.MILD) then
			self:SetMildLength (season_length)
	
	elseif(season == SEASONS.WET) then
			self:SetWetLength (season_length)
	
	elseif(season == SEASONS.GREEN) then
			self:SetGreenLength (season_length)
	
	elseif(season == SEASONS.DRY) then
			self:SetDryLength (season_length)

	elseif(season == SEASONS.TEMPERATE) then
			self:SetTemperateLength (season_length)

	elseif(season == SEASONS.HUMID) then
			self:SetHumidLength (season_length)

	elseif(season == SEASONS.LUSH) then
			self:SetLushLength (season_length)

	elseif(season == SEASONS.APORKALYPSE) then
			self:SetAporkalypseLength (season_length)
	else
		--error (season)
	end
...
function SeasonManager:StartSeasonO(season)
	--To allow for a season cycle to have different lengths for having mutiple seasons of the same type.
	self:UpdateLength(self.seasonlist[self.season_current_number+1].season_title, self.seasonlist[self.season_current_number+1].season_length)
	print(DebugSeasonList(self.seasonlist))
...
function SeasonManager:UpdateSegs()
	...
	local function get_season(seasonlist, currentseason)
		if seasonlist[currentseason+1].season_title == nil then
			return currentseason
		end
		local season = seasonlist[currentseason+1].season_title
		local i, n = 0, #seasonlist
		while i < n and seasonlist[season].season_title and (self:GetSeasonLength(season) <= 0 or self:GetSeasonIsEnabled(season) == false) do
			season = seasonlist[season].season_title
			i = i + 1
		end
		return season
	end
	local function getNextSeason()
	
		local count = 0
		local season_number_temp = self.season_current_number + 1
		
		if season_number_temp > self.season_limit_number then
			season_number_temp = 0
		end
		while self.seasonlist[season_number_temp+1].season_title == "none" do
		
			season_number_temp = season_number_temp + 1
			if season_number_temp > self.season_limit_number then
				season_number_temp = 0
			
			end
			count = count + 1
			if count > self.season_limit_number then
					error("something went wrong getNextSeason!")
			end
		end
	
		return self.seasonlist[season_number_temp+1].season_title
	end
	local function getPreSeason()

		local count = 0
		local season_number_temp = self.season_current_number - 1
		
		if season_number_temp < 0 then
				season_number_temp = self.season_limit_number			
		end
		...
		while self.seasonlist[season_number_temp+1].season_title == "none" do
			...
			count = count + 1
			if count > self.season_limit_number then
				error("something went wrong getpreSeason!")
			end
	...
	local nSeason = getNextSeason()	
	local pSeason = getPreSeason()
	if self.current_season == nil then
		error("error current!")
	end
	...
	if self.aporkalypse_transition then
		...
		if 	--[[(self.current_season == SEASONS.APORKALYPSE) or]] (aporkalypse and aporkalypse:IsActive()) then

			--if( (not self.pre_aporkalypse_season) or (self.pre_aporkalypse_season ~= SEASONS.APORKALYPSE)) then
...
function SeasonManager:SetAutumnLength(Len)
	...
	self.autumnlength = Len
	self.autumnenabled = Len > 0
end

function SeasonManager:SetWinterLength(Len)

	self.winterlength = Len
	self.winterenabled = Len > 0
end

function SeasonManager:SetSpringLength(Len)
	
	self.springlength = Len
	self.springenabled = Len > 0
end

function SeasonManager:SetSummerLength(Len)

	self.summerlength = Len
	self.summerenabled = Len > 0
end
...
function SeasonManager:OnSave()
	...
	-- SAVING SEASON CHANGER CHECK --
	return
    {
		season_current_number = self.season_current_number,
		seasonlist = deepcopy(self.seasonlist), -- !seasonLimit
		season_mod_enabled = 1, --Mod is enabled
...
function DebugSeasonList(seasonlist)
   print("DEBUG PRINT: SEASONLIST") --DEBUG
   for k,v in pairs(seasonlist) do
		print(seasonlist[k].season_title)
		print(seasonlist[k].season_length)
	end
end

function SeasonManager:DebugLength(season_index, length)
	self.seasonlist[season_index].season_length = length
	print(DebugSeasonList(self.seasonlist)) 
end

function SeasonManager:ConvertOldMap()
	return
--[[ EXTRAS: Check function later for the !override option
	if( self.seasonmode == "tropical") then

		self.t_season_1 = SEASONS.MILD
		self.t_season_2 = SEASONS.WET
		self.t_season_3 = SEASONS.GREEN
		self.t_season_4 = SEASONS.DRY
		self.t_season_5 = "none"
		self.t_season_6 = "none"
		self.t_season_7 = "none"

		self.t_s1_length = self.mildlength
		self.t_s2_length = self.wetlength
		self.t_s3_length = self.greenlength
		self.t_s4_length = self.drylength
		self.t_s5_length = 0
		self.t_s6_length = 0
		self.t_s7_length = 0
		...
		if( self.current_season == SEASONS.MILD) then
			self.season_current_number = 0
		elseif( self.current_season == SEASONS.WET) then
			self.season_current_number = 1
		elseif( self.current_season == SEASONS.GREEN) then
			self.season_current_number = 2
		else
			self.season_current_number = 3
		end
		
		
	elseif( self.seasonmode == "plateau") then

		self.t_season_1 = SEASONS.TEMPERATE
		self.t_season_2 = SEASONS.HUMID
		self.t_season_3 = SEASONS.LUSH
		self.t_season_4 = "none"
		self.t_season_5 = "none"
		self.t_season_6 = "none"
		self.t_season_7 = "none"

		self.t_s1_length = self.temperatelength
		self.t_s2_length = self.humidlength
		self.t_s3_length = self.lushlength
		self.t_s4_length = 0
		self.t_s5_length = 0
		self.t_s6_length = 0
		self.t_s7_length = 0

		if( self.current_season == SEASONS.TEMPERATE) then
			self.season_current_number = 0
		elseif( self.current_season == SEASONS.HUMID) then
			self.season_current_number = 1		
		else
		...
		self.t_season_1 = SEASONS.AUTUMN
		self.t_season_2 = SEASONS.WINTER
		self.t_season_3 = SEASONS.SPRING
		self.t_season_4 = SEASONS.SUMMER
		self.t_season_5 = "none"
		self.t_season_6 = "none"
		self.t_season_7 = "none"

		self.t_s1_length = self.autumnlength
		self.t_s2_length = self.winterlength
		self.t_s3_length = self.springlength
		self.t_s4_length = self.summerlength
		self.t_s5_length = 0
		self.t_s6_length = 0
		self.t_s7_length = 0
...
function SeasonManager:OnLoad(data)
	...
	self.season_current_number = data.season_current_number or self.season_current_number
	
	if data.seasonlist then
		for k,v in pairs(data.seasonlist) do
			print("DEBUG LOADING: SAVEDATA SEASONLIST") --DEBUG
			print(data.seasonlist[k].season_title) --DEBUG
			print(data.seasonlist[k].season_length)
			self.seasonlist[k] = data.seasonlist[k]
			print("DEBUG LOADING: SEASONLIST") --DEBUG
			print(self.seasonlist[k].season_title) --DEBUG
			print(self.seasonlist[k].season_length) --DEBUG
		end
	end
	
	self.season_mod_enabled = data.season_mod_enabled or 0
	...
	if(	self.season_mod_enabled == 0) then -- if we load a map not saved with this mod
		self:ConvertOldMap() -- Where !override can be called
		print("Seasons Changer mod: Save Convertido")
	end
	...
	for i=1, self.season_limit_number+1, 1 do 
		self:UpdateLength(self.seasonlist[i].season_title,self.seasonlist[i].season_length)
		print("DEBUG UPDATE: SEASON LENGTH") --DEBUG
		print(self.seasonlist[i].season_length) --DEBUG
	end
	--Make sure mutiple seasons of the same type set their proper length onload.
	self:UpdateLength(self.seasonlist[self.season_current_number+1].season_title,self.seasonlist[self.season_current_number+1].season_length)
...
function SeasonManager:GetPeakIntensity() -- EXTRAS: Does this determine how extreme a season can be?
...
function SeasonManager:StartCavesRain()
	...
	--[[ BUGS: The OG mod doesn't have this issue with cave loading.
...
function SeasonManager:OnUpdate( dt )	
	--print ("time to pass stststststst:", dt)
...
--		print("SEASON TEMP =",season_temp, percent_season)
	...
	--else
		if self.current_season == SEASONS.WINTER then
			...
			--print ("Winter temp stststststst")
			--print (season_temp)
		elseif self.current_season == SEASONS.SPRING then
		...
		elseif self.current_season == SEASONS.AUTUMN then
			if GetClock():GetNumCycles() < 10 then
...
--	print("day effect",time_temp)
	...
	----------------------- Temperature changer smoothed ---------------------
	local MAX_VARIATION_DAY = 1/20

	local tmp_temperature = temperature_noise + season_temp + time_temp
	
	if(self.current_temperature_init == 0) then --Do not apply this when loading the game
		self.current_temperature = temperature_noise + season_temp + time_temp
		self.current_temperature_init = 1
	else
		if( math.abs(tmp_temperature - self.current_temperature_last) > MAX_VARIATION_DAY*dt) then
			
			if(tmp_temperature > self.current_temperature_last) then
				local dif_temp =  (tmp_temperature-self.current_temperature_last) * dt
				if( dif_temp > (MAX_VARIATION_DAY*dt)) then
					dif_temp = MAX_VARIATION_DAY*dt
				end
				self.current_temperature = 	math.min(tmp_temperature, self.current_temperature_last + dif_temp)			
			elseif(tmp_temperature < self.current_temperature_last) then
				local dif_temp =  (self.current_temperature_last-tmp_temperature) * dt
				if( dif_temp > (MAX_VARIATION_DAY*dt)) then
					dif_temp = MAX_VARIATION_DAY*dt
				end
				self.current_temperature = 	math.max(tmp_temperature, self.current_temperature_last - dif_temp)	
			end
		else
			self.current_temperature = temperature_noise + season_temp + time_temp
		end	
	...
	self.current_temperature_last = self.current_temperature
...
--		    	print("rain rain go away" .. " " .. self.precip_rate)
		...
		if self.current_season == SEASONS.SUMMER and not self.incaves then
