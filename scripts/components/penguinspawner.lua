...
local LAND_CHECK_RADIUS = 3
local WATER_CHECK_RADIUS = 6
...
function FindLandNextToWater( playerpos, waterpos )
	...
	local test = function(offset)
        ...
        if tile == GROUND.IMPASSABLE or tile >= GROUND.UNDERGROUND or (tile >= GROUND.MANGROVE and tile <= GROUND.LILYPOND) then
			return false
		end
...
function PenguinSpawner:SpawnFlock(colonyNum,loc,check_angle)
    ...
    while spawned < flock and i < flock + 7 do
        ...
        i = i + 1
        if not (tile == GROUND.IMPASSABLE or tile >= GROUND.UNDERGROUND or (tile >= GROUND.MANGROVE and tile <= GROUND.LILYPOND)) then
            spawned = spawned + 1
